...
Orgo Membership Fees is the dues collection and billing module within Orgo that lets organizations charge, track, and reconcile recurring and one-time member payments.

**Built for** membership-based organizations that collect dues from a distributed member base, including professional associations, alumni networks, youth organizations, and faith communities, and similar groups managing member contributions.

**Replaces** spreadsheets and manual invoicing for tracking who has paid, not for full accounting or bookkeeping (payment exports feed into those systems).

Fees handles annual dues, monthly subscriptions, and one-time payments through Stripe or manual recording (cash/bank transfers), with a dashboard showing who's paid and who hasn't.
...
Each [local center](/platform/groups/local-groups) can have its own fee product with independent pricing and its own Stripe Connect account. This supports federated organizations where chapters collect their own dues.
