...
A template only fills fields you have not already set on the invoice, so you can
apply a template and then override any single value. On invoices Orgo generates
for membership payments the template's flat tax rate is ignored, because each
line carries the VAT rate from its own price.
