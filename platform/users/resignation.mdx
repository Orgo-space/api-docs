...
"og:description": 'Formal member departure with reason tracking and admin approval'
"twitter:title": 'Resignation | Orgo'
"twitter:description": 'Formal member departure with reason tracking and admin approval'
---
...
## Requests that sit waiting

Every resignation needs an administrator to approve it, which means a request can
sit pending indefinitely if nobody is watching the queue. Members experience that
as being ignored at the moment they have already decided to leave, and they tend
to escalate by emailing whoever they can find.

Two habits prevent it:

- Check the pending queue on a schedule rather than when someone chases
- Assign the queue to a named person, not to "the admins"

<Note>
A member who says the resignation flow did not work has often submitted a request
that is still pending, rather than hit an error. Check the queue before treating
it as a fault. If there is genuinely no request there, take their email as the
notice, record the resignation on their behalf, and report the failure.
</Note>

## Resignation and data deletion are different requests

Resigning ends membership. It does not erase anything, and it should not: you
keep the record, the history, and the reason they left.

A member asking for their **data to be deleted** is making a separate request
under the right to erasure. Handle it through account deletion, not by resigning
them and assuming that covers it. Someone who asked to be erased and later
discovers their profile still exists has a legitimate complaint, and in the EU it
is a regulatory one rather than a service one.
