...
money, `HR_LOCAL` for the export and the status column. If they also need the
fee filters, that is `FINANCIAL_TENANT`, which reaches every chapter, so consider
whether the chapter-scoped screens in
[Local group fees](/platform/local-group-fees/member-fees) are the better answer
...
<AccordionGroup>
  ...
  <Accordion title="The New requests (N) button never appears">
    ...
    `HR_LOCAL` alone does not show the counter, and it does not show the
    **Approve** action on the row either. Approving members is `ADMIN_LOCAL` and
    above.
  </Accordion>
