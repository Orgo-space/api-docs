...
The email log answers one question well: did we send it, and what did it say. Every
email the platform sends is recorded with its recipient, subject, time and the
exact rendered body.
...
| **Type** | Which kind of email it was |
...
with their name, dates and links already substituted.
...
<Steps>
  ...
  <Step title="No row at all?">
    Orgo did not send it. Either the email is switched off (check
    [Emails Config](/platform/emails/system-emails)) or the thing that triggers it
    did not happen: no payment recorded, no invitation issued, no event
    registration. Fix the cause, not the email.
  </Step>
  <Step title="A row exists?">
    Orgo sent it. Check the address on the row: an old address, a typo, or a
    placeholder address left by a data import all produce a row that was never
    going to arrive.
  </Step>
