...
The queue is at `/discuss/moderation-queue`. On a local center, a **Moderation queue** button with a pending count appears in the header next to Info.
...
<AccordionGroup>
  ...
  <Accordion title="I cannot find the moderation queue">
    The button only appears in a local center's header. Open `/discuss/moderation-queue` directly for every other scope. If both tables are empty, either there is nothing held or you do not hold moderation rights for any group with held posts.
  </Accordion>
