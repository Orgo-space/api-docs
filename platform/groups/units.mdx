...
The segmented board is the view you will spend most time in: one column per unit, the members of each shown as cards, and an **Unassigned** column holding everyone in the chapter who is not in a unit yet. You move people by dragging a card from one column to another.
...
|---|---|---|---|
| **Chapter** (local center) | The organization | The member's primary chapter, one per member | `ADMIN_TENANT` |
...
| Requirement | Where |
|---|---|
| **Enable Organizational Units** switched on | **Settings** → **Modules** → **Groups** → **Configuration** |
| At least one unit type defined | **Settings** → **Modules** → **Groups** → **Unit Types** |
...
A unit type is the template for a layer of your structure. Define them once at **Settings** → **Modules** → **Groups** → **Unit Types**, then create units of that type inside each chapter.
...
`ADMIN_LOCAL` also gets a drag-and-drop board for the whole chapter. When your types nest, it renders as columns: one per parent unit, with its child units stacked inside and an **Unassigned** column for members who are in the chapter but not in any unit.
...
| Create a unit | `HR_LOCAL`. `HR_TENANT` may create any type; local admins are limited to ordinary units |
