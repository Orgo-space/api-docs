...
"og:description": 'Sell tickets with multiple price tiers, member discounts, and Stripe checkout'
"twitter:title": 'Event Ticketing | Orgo'
"twitter:description": 'Sell tickets with multiple price tiers, member discounts, and Stripe checkout'
---

Orgo Event Ticketing is a built-in ticketing module within the Orgo membership platform that lets organizations sell event tickets with member pricing, capacity limits, add-ons, and QR code check-in.

**Built for** membership organizations running paid events for members and non-members, including professional associations, youth organizations, alumni networks, and trade unions, and similar member-based groups coordinating events from small workshops to large conferences across chapters.

**Replaces** separate tools like Eventbrite, Ticket Tailor, or Cvent with ticketing tied directly to your member database and payment processing, with no per-ticket fees from Orgo, only Stripe's standard rate.

Attendees pay via Stripe, receive a QR code ticket by email, and check in at the door by scanning it.
...
<Steps>
  ...
  <Step title="Save">
    Tickets appear on the [event page](/platform/events/public-page) immediately.
  </Step>
</Steps>
...
<Note>
  QR scanning requires **HR Tenant** [permissions](/platform/permissions) or higher. Any admin with event management access can check people in.
</Note>
...
| **External speaker** | Select from your [contacts](/platform/contacts) list — name and bio from the contact record |
